...
def account_login(client, url, email, password):
    ...
    client.post(f'https://appdrive.in/login', data=data)
...
def update_account(client, url, shared_drive_id, folder_id):
    ...
    client.post(f'https://appdrive.in/account', data=data)
...
def appdrive_dl(url):
    ...
    if url == 'appdrive.in' and not info_parsed['error']:
        res = client.get(info_parsed['gdrive_link'])
        drive_link = etree.HTML(res.content).xpath("//a[contains(@class,'btn')]/@href")[0]
        info_parsed['gdrive_link'] = drive_link
    bypassed = info_parsed['gdrive_link']
    return bypassed
